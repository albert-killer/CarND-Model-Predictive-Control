...
// TODO: Tune here!
size_t N = 9;
// the smaller the heavier to compute
double dt = 0.1;
...
// --> Lf will change with size of car!
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    // This function is used by the MPC solver
    ...
    // COST FUNCTION --> TUNE HERE!:
    // The part of the cost based on the reference state.
    // TODO: Tune the influence of each error by multiplying with certain factors
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      ...
      // Remember: dealing with a 3rd order polynomial here
      AD<double> f0 = coeffs[0] + coeffs[1]*x0 + coeffs[2]*pow(x0, 2) + coeffs[3]*pow(x0, 3);
      AD<double> psides0 = CppAD::atan(coeffs[1] + 2*coeffs[2]*x0 + 3*coeffs[3]*pow(x0, 2));

      // Setup the model constraints:
      // Important: The idea here is to constraint this value (fg) to be 0.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // 4 * 10 + 2 * 9
  // here: 25 * 6 + (25 - 1) * 2 = 198
  // hint: N-1 for delta and a because you don't have to worry about the endpoints for both of them
  ...
  // TODO: Return the first actuator values.
  // The variables can be accessed with `solution.x[i]`.

  // Define vector which can be red by "steer_value" in main.cpp
  ...
  for (unsigned int i = 0; i < N-1; i++){
    // Passing the future predictions for visualization (green line)
